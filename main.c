...
int main(){
    ...
    if ((da = fopen("dados.txt", "r")) != NULL){
        for(j = v; j < v + n; j++){	
            fscanf(da, "%i", &j->data.dia);
            fscanf(da, "%i", &j->data.mes);
            fscanf(da, "%i", &j->data.ano);
            fscanf(da, "%i", &j->hInicio.hora);
            fscanf(da, "%i", &j->hInicio.minuto);
            fscanf(da, "%i", &j->hFim.hora);
            fscanf(da, "%i", &j->hFim.minuto);
            fscanf(da, "%s", j->descricao);
            fscanf(da, "%s", j->local);
            fscanf(da, "$", a);
        }
    }
